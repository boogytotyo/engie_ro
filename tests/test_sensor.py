...
async def test_sensor_values(hass: HomeAssistant, entry: ConfigEntry):
    with patch("custom_components.engie_ro.api.EngieApiClient.load_token", new=AsyncMock(return_value="TOK")),          patch("custom_components.engie_ro.api.EngieApiClient.fetch_account_overview", new=AsyncMock(return_value={
            "pa":"123","poc":"5001","division":"gaz"
         })),          patch("custom_components.engie_ro.api.EngieApiClient.fetch_current_index", new=AsyncMock(return_value={
            "value":"111.2","unit":"kWh","timestamp":"2025-09-01T10:00:00"
         })),          patch("custom_components.engie_ro.api.EngieApiClient.fetch_billing_history", new=AsyncMock(return_value={
            "items":[{"id":"B9","issue_date":"2025-07-01","due_date":"2025-07-20","amount":88.50,"currency":"RON","status":"unpaid"}]
         })),          patch("custom_components.engie_ro.coordinator.EngieCoordinator.async_config_entry_first_refresh", new=AsyncMock()):
        coord = await create_coordinator(hass, entry, timedelta(seconds=10))
        hass.data.setdefault(DOMAIN, {})[entry.entry_id] = coord

        from homeassistant.const import Platform
        with patch("homeassistant.config_entries.ConfigEntries.async_forward_entry_setups", new=AsyncMock()):
            assert await async_setup_entry(hass, entry)

        coord.data = {
            "overview": {"pa":"123","poc":"5001","division":"gaz"},
            "current_index": {"value":111.2,"unit":"kWh","timestamp":"2025-09-01T10:00:00"},
