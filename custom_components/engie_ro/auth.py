...
class EngieAuthManager:
    ...
    async def _write_token_to_file(self, bundle: dict) -> None:
        try:
            ...
            await asyncio.to_thread(
                tmp.write_text, json.dumps(bundle, ensure_ascii=False, indent=0), encoding="utf-8"
            )
    ...
    async def _login_mobile(self) -> str:
        ...
        token, refresh_token, exp_sec, refresh_epoch = await self.client.mobile_login(
    ...
    async def ensure_valid_token(self) -> str:
        ...
        if self.auth_mode == AUTH_MODE_MOBILE:
            ...
            raise EngieUnauthorized(
                "Bearer token missing/invalid. Provide a valid token or switch to mobile login."
            )
