    ...
    async def _async_update_data(self) -> dict[str, Any]:
        try:
            ...
            contract_account = _find_first(
                places, ["contract_account", "contractAccount", "ca", "accountNumber"]
            )
            contract_account_number = _find_first(
                places, ["contract_account_number", "contractAccountNumber"]
            )
            ...
            try:
                if poc_number:
                    idx_payload = await self.client.get_index_window(
            ...
            try:
                ...
                if poc_number and pa:
                    inv_hist = await self.client.get_invoices_history(
            ...
            try:
                if poc_number:
                    hist = await self.client.get_invoices_history(
                        poc_number,
                        division=division,
                        pa=pa,
            ...
            try:
                ...
                start_date = (today - timedelta(days=365)).strftime("%Y-%m-%d")
                if poc_number and pa:
                    cons = await self.client.get_consumption(
                        poc_number, start_date, end_date, pa=pa
                    )
                    items = []
                    if isinstance(cons, dict):
                        ...
                        for month_item in arr:
                            ...
                            for inv in invs:
                                d = str(
                                    inv.get("invoiced_at") or month_item.get("invoiced_at") or ""
                                )
                                amount = (
                                    inv.get("consum_gaz")
                                    or inv.get("value")
            ...
            try:
                ...
                if poc_number and division and index_info:
                    ...
                    hist = await self.client.get_index_history_post(
                        ...
                        for it in arr:
                            ...
                            if dt and dt.year == this_year:
                                if idx_num is not None:
                                    index_history_list.append(
                                        f"{dt.strftime('%d.%m.%Y')}\t{idx_num}"
                                    )
