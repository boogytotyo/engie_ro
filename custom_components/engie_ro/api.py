...
class EngieClient:
    ...
    async def get_balance(self, contract_account: str) -> Any:
        try:
            return await self._post_form_json("/v1/widgets/ballance", {"contract_account[]": (contract_account or "").strip()})
        except EngieHTTPError:
            return await self._post_form_json("/v1/widgets/ballance", {"contract_account": (contract_account or "").strip()})

    async def get_invoices_details(self, contract_account: str) -> Any:
        try:
            return await self._post_form_json("/v1/invoices/ballance-details", {"contract_account[]": (contract_account or "").strip()})
        except EngieHTTPError:
            return await self._post_form_json("/v1/invoices/ballance-details", {"contract_account": (contract_account or "").strip()})
